...
// ============================================================================
// Priority 1: Pronoun Resolution Tests (M8 Polish)
// ============================================================================

/// Helper to create an event with a female agent
fn create_female_event(predicate: &str, agent_name: &str, aspect: AspectualClass) -> ComposedEvent {
    let agent = Entity {
        id: 1,
        text: agent_name.to_string(),
        animacy: Some(Animacy::Human),
...
#[test]
fn test_pronoun_she_resolution() {
    // Uses process_event which auto-infers gender from name "Mary"
    let mut ctx = DiscourseContext::with_defaults();

    // Sentence 1: "Mary runs." - gender inferred from name
    ctx.begin_sentence("Mary runs.".to_string());
    let event1 = create_female_event("run", "Mary", AspectualClass::Activity);
    ctx.process_event(&event1).expect("Should process event");
    ctx.end_sentence();

    // Sentence 2: Resolve "she"
    ctx.begin_sentence("She smiles.".to_string());
...
#[test]
fn test_pronoun_it_resolution_inanimate() {
    use canopy_discourse::{Gender, ReferentType};

    let mut ctx = DiscourseContext::with_defaults();

    // Sentence 1: Introduce an inanimate object (no name-based gender)
    ctx.begin_sentence("The book fell.".to_string());
    let book_id = ctx
        .introduce_referent("the book".to_string(), ReferentType::Individual)
        .expect("Should introduce book");

    // Set neuter gender for inanimate
    if let Some(referent) = ctx.referents_mut().get_mut(book_id) {
        referent.set_gender(Gender::Neuter);
    }

    ctx.end_sentence();

    // Sentence 2: Resolve "it"
    ctx.begin_sentence("It was heavy.".to_string());

    let resolved = ctx.resolve_pronoun("it");
    assert!(resolved.is_ok(), "Should resolve 'it' to the book");
    assert_eq!(
        resolved.unwrap(),
        book_id,
        "'it' should resolve to the book's referent ID"
    );
...
#[test]
fn test_pronoun_they_plural_resolution() {
    use canopy_discourse::{NumberFeature, ReferentType};

    let mut ctx = DiscourseContext::with_defaults();

    // Sentence 1: Introduce a plural referent
    ctx.begin_sentence("The students arrived.".to_string());
    let students_id = ctx
        .introduce_referent("the students".to_string(), ReferentType::Individual)
        .expect("Should introduce students");

    // Set plural number
    if let Some(referent) = ctx.referents_mut().get_mut(students_id) {
        referent.set_number(NumberFeature::Plural);
    }

    ctx.end_sentence();

    // Sentence 2: Resolve "they"
    ctx.begin_sentence("They were tired.".to_string());

    let resolved = ctx.resolve_pronoun("they");
    assert!(resolved.is_ok(), "Should resolve 'they' to the students");
    assert_eq!(
        resolved.unwrap(),
        students_id,
        "'they' should resolve to the students' referent ID"
    );
...
#[test]
fn test_pronoun_resolution_most_recent_wins() {
    // Use process_event which auto-infers gender from names
    let mut ctx = DiscourseContext::with_defaults();

    // Sentence 1: "John entered."
    ctx.begin_sentence("John entered.".to_string());
    let event1 = create_test_event("enter", "John", AspectualClass::Achievement);
    ctx.process_event(&event1).expect("Should process event 1");
    ctx.end_sentence();

    // Sentence 2: "Bob followed." (Bob is more recent)
    ctx.begin_sentence("Bob followed.".to_string());
    let event2 = create_test_event("follow", "Bob", AspectualClass::Activity);
    ctx.process_event(&event2).expect("Should process event 2");
    ctx.end_sentence();

    // Sentence 3: Resolve "he" - should prefer Bob (more recent/salient)
    ctx.begin_sentence("He smiled.".to_string());

    let resolved = ctx.resolve_pronoun("he");
    assert!(resolved.is_ok(), "Should resolve 'he'");

    // Verify we resolved to something - recency should favor Bob
    ctx.end_sentence();
...
#[test]
fn test_pronoun_gender_filtering() {
    // Use process_event which auto-infers gender from names "John" and "Mary"
    let mut ctx = DiscourseContext::with_defaults();

    // Introduce both genders via transitive event
    ctx.begin_sentence("John met Mary.".to_string());
    let event = create_transitive_event("meet", "John", "Mary", AspectualClass::Achievement);
    ctx.process_event(&event).expect("Should process event");
    ctx.end_sentence();

    // "she" should only match Mary, not John
    ctx.begin_sentence("She waved.".to_string());
    let resolved_she = ctx.resolve_pronoun("she");
    assert!(
        resolved_she.is_ok(),
        "Should resolve 'she': {:?}",
        resolved_she
    ...
    // "he" should only match John, not Mary
    ctx.begin_sentence("He nodded.".to_string());
    let resolved_he = ctx.resolve_pronoun("he");
    assert!(
        resolved_he.is_ok(),
        "Should resolve 'he': {:?}",
        resolved_he
    ...
    let stats = ctx.statistics();
    assert_eq!(
        stats.resolution_count, 2,
        "Should have 2 anaphora resolutions (she + he)"
    );
}

// ============================================================================
// Priority 5: Edge Case Tests (M8 Polish)
// ============================================================================

#[test]
fn test_empty_discourse_statistics() {
...
#[test]
fn test_empty_discourse_drs() {
    let ctx = DiscourseContext::with_defaults();

    let drs = ctx.drs();
    assert!(drs.entity_referents().is_empty());
    assert!(drs.event_referents().is_empty());
    assert!(drs.conditions.is_empty());
}

#[test]
fn test_pronoun_resolution_no_candidates_fails() {
    let mut ctx = DiscourseContext::with_defaults();

    // Start a sentence with no prior referents
    ctx.begin_sentence("He arrived.".to_string());

    // Try to resolve "he" with no masculine referents available
    let result = ctx.resolve_pronoun("he");

    // Should fail - no antecedent candidates
    assert!(
        result.is_err(),
        "Should fail to resolve 'he' with no candidates: {:?}",
        result
    );

    ctx.end_sentence();

    // Resolution count should be 0 (failed resolution doesn't count)
    let stats = ctx.statistics();
    assert_eq!(
        stats.resolution_count, 0,
        "Failed resolution should not increment count"
    );
}

#[test]
fn test_pronoun_resolution_wrong_gender_fails() {
    let mut ctx = DiscourseContext::with_defaults();

    // Introduce only Mary (female)
    ctx.begin_sentence("Mary runs.".to_string());
    let event = create_female_event("run", "Mary", AspectualClass::Activity);
    ctx.process_event(&event).expect("Process event");
    ctx.end_sentence();

    // Try to resolve "he" - should fail (no masculine referents)
    ctx.begin_sentence("He smiled.".to_string());

    let _result = ctx.resolve_pronoun("he");
    // May succeed if there's a fallback, or fail if strict gender matching
    // The key test is that "she" would definitely work
    ctx.end_sentence();

    // Now verify "she" works
    ctx.begin_sentence("She laughed.".to_string());
    let she_result = ctx.resolve_pronoun("she");
    assert!(
        she_result.is_ok(),
        "'she' should resolve to Mary: {:?}",
        she_result
    );
    ctx.end_sentence();
}
...
#[test]
fn test_sentence_without_events() {
    let mut ctx = DiscourseContext::with_defaults();

    // Begin and end sentence without processing any events
    ctx.begin_sentence("Empty sentence.".to_string());
    ctx.end_sentence();

    let stats = ctx.statistics();
    assert_eq!(stats.sentence_count, 1, "Should count the sentence");
    assert_eq!(stats.referent_count, 0, "Should have no referents");
}
...
fn test_multiple_clear_cycles() {
    ...
    // Cycle 2: Add and clear again
    ctx.begin_sentence("Mary walks.".to_string());
    let event2 = create_female_event("walk", "Mary", AspectualClass::Activity);
    ctx.process_event(&event2).expect("Process event");
    ctx.end_sentence();

    let stats = ctx.statistics();
    assert_eq!(
        stats.sentence_count, 1,
        "Should only count post-clear sentence"
    );

    ctx.clear();
    let final_stats = ctx.statistics();
    assert_eq!(final_stats.sentence_count, 0);
    assert_eq!(final_stats.referent_count, 0);
}

#[test]
fn test_event_without_sentence_context() {
    let mut ctx = DiscourseContext::with_defaults();

    // Don't call begin_sentence - process event directly
    // This tests defensive behavior
    let event = create_test_event("run", "John", AspectualClass::Activity);
    let result = ctx.process_event(&event);

    // Should still work (or fail gracefully)
    // The exact behavior depends on implementation
    // Key is no panic
    if result.is_ok() {
        // If it succeeds, verify referent was created
        assert!(ctx.statistics().referent_count > 0);
    }
...
#[test]
fn test_drs_conditions_grow_with_events() {
    let mut ctx = DiscourseContext::with_defaults();

    ctx.begin_sentence("John runs.".to_string());
    let initial_conditions = ctx.drs().condition_count();

    let event1 = create_test_event("run", "John", AspectualClass::Activity);
    ctx.process_event(&event1).expect("Process event 1");

    let after_first = ctx.drs().condition_count();
    assert!(
        after_first > initial_conditions,
        "Conditions should increase after first event"
    );

    let event2 = create_test_event("jump", "John", AspectualClass::Achievement);
    ctx.process_event(&event2).expect("Process event 2");

    let after_second = ctx.drs().condition_count();
    assert!(
        after_second > after_first,
        "Conditions should increase after second event"
    );
...
#[test]
fn test_referent_introduction_with_empty_name() {
    use canopy_discourse::ReferentType;

    let mut ctx = DiscourseContext::with_defaults();

    ctx.begin_sentence("Something happened.".to_string());

    // Introduce referent with empty string (edge case)
    let _result = ctx.introduce_referent("".to_string(), ReferentType::Individual);

    // Should handle gracefully - either succeed or fail, but no panic
    // Empty name is unusual but should be handled
    ctx.end_sentence();
}

#[test]
fn test_temporal_relation_between_single_event() {
    let mut ctx = DiscourseContext::with_defaults();

    ctx.begin_sentence("John ran.".to_string());
    let event = create_test_event("run", "John", AspectualClass::Activity);
    ctx.process_event(&event).expect("Process event");
    ctx.end_sentence();

    // With only one event, there should be no temporal relations
    let temporal_count = ctx
        .drs()
        .conditions
        .iter()
        .filter(|c| matches!(c, DrsCondition::TemporalRelation { .. }))
        .count();

    assert_eq!(
        temporal_count, 0,
        "Single event should have no temporal relations"
    );
}

#[test]
fn test_config_with_zero_capacity() {
    // Edge case: what happens with very small capacity?
    ...
    assert_eq!(ctx.statistics().sentence_count, 1);
}
