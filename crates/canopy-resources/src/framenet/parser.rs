...
use super::types::{
    CoreType, Frame, FrameElement, FrameElementRealization, FrameElementRelation, FrameRelation,
    Lexeme, LexicalUnit, LexicalUnitRef, SemanticType, ValencePattern,
};
use crate::engine::{
    extract_text_content, get_attribute, skip_element, EngineError, EngineResult, XmlResource,
};
...
/// Extract frame attributes from start tag
fn extract_frame_attrs(e: &quick_xml::events::BytesStart, frame: &mut Frame) {
    if let Some(id) = get_attribute(e, "ID") {
        frame.id = id;
    }
    if let Some(name) = get_attribute(e, "name") {
        debug!("Parsing FrameNet frame: {name}");
        frame.name = name;
    }
...
/// Handle empty frame elements (self-closing tags)
fn handle_frame_empty_element(e: &quick_xml::events::BytesStart, frame: &mut Frame) {
    match e.name() {
        QName(b"frameRelation") => {
            frame.frame_relations.push(FrameRelation {
                relation_type: get_attribute(e, "type").unwrap_or_default(),
                related_frame_id: get_attribute(e, "relatedFrame").unwrap_or_default(),
                related_frame_name: get_attribute(e, "relatedFrameName").unwrap_or_default(),
            });
        }
        QName(b"lexUnit") => {
            frame.lexical_units.push(LexicalUnitRef {
                ...
                pos: get_attribute(e, "POS").unwrap_or_default(),
                status: get_attribute(e, "status").unwrap_or_default(),
            });
        }
        _ => trace!("Skipping unknown empty element: {:?}", e.name()),
    }
}
        ...
        loop {
            match reader.read_event_into(&mut buf) {
                Ok(Event::Start(ref e)) => match e.name() {
                    QName(b"frame") => extract_frame_attrs(e, &mut frame),
                    QName(b"definition") => {
                    ...
                    QName(b"FE") => {
                        ...
                        frame
                            .frame_elements
                            .push(parse_frame_element(reader, &mut fe_buf, e)?);
                    }
                    QName(b"frameRelation") => {
                        let mut rel_buf = Vec::new();
                        frame
                            .frame_relations
                            .push(parse_frame_relation(reader, &mut rel_buf, e)?);
                    }
                    QName(b"lexUnit") => {
                        let mut lu_buf = Vec::new();
                        frame
                            .lexical_units
                            .push(parse_lexical_unit_ref(reader, &mut lu_buf, e)?);
                    }
                    _ => trace!("Skipping unknown element: {:?}", e.name()),
                },
                Ok(Event::Empty(ref e)) => handle_frame_empty_element(e, &mut frame),
...
/// Extract attributes from frame element start tag
fn extract_fe_attributes(start_tag: &quick_xml::events::BytesStart) -> FrameElement {
    FrameElement {
        id: get_attribute(start_tag, "ID").unwrap_or_default(),
        name: get_attribute(start_tag, "name").unwrap_or_default(),
        abbrev: get_attribute(start_tag, "abbrev").unwrap_or_default(),
        core_type: get_attribute(start_tag, "coreType").map_or(CoreType::Core, |ct| {
            match ct.as_str() {
        ...
        bg_color: get_attribute(start_tag, "bgColor"),
        fg_color: get_attribute(start_tag, "fgColor"),
        created_by: get_attribute(start_tag, "cBy"),
        created_date: get_attribute(start_tag, "cDate"),
        definition: String::new(),
...
/// Handle empty element inside FE
fn handle_fe_empty_element(e: &quick_xml::events::BytesStart, fe: &mut FrameElement) {
    match e.name() {
        QName(b"semType") => {
            let name = get_attribute(e, "name").unwrap_or_default();
...
fn parse_frame_element<R: BufRead>(
    ...
    let mut fe = extract_fe_attributes(start_tag);
    ...
    loop {
        match reader.read_event_into(buf) {
            ...
            Ok(Event::Empty(ref e)) => handle_fe_empty_element(e, &mut fe),
...
// XML utilities (get_attribute, extract_text_content, skip_element)
// imported from canopy_engine::xml_utils
