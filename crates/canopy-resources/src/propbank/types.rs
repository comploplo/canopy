...
impl ArgumentModifier {
    ...
    pub fn from_propbank_label(s: &str) -> Self {
        s.parse().unwrap_or_else(|()| Self::Other(s.to_string()))
    }
}
...
impl PropBankAnalysis {
    ...
    pub fn calculate_confidence(&mut self) {
        if let Some(ref pred) = self.predicate {
            ...
            let core_args = count_to_f32(pred.get_core_arguments().len());
            let modifier_args = count_to_f32(pred.get_modifiers().len());
...
mod tests {
    ...
    fn test_propbank_argument_new() {
        ...
        assert!((arg.confidence - 0.9).abs() < f32::EPSILON);
    }
    ...
    fn test_propbank_argument_with_span() {
        ...
        assert!((arg.confidence - 0.85).abs() < f32::EPSILON);
    }
    ...
    fn test_propbank_analysis_calculate_confidence() {
        ...
        assert!((with_alt.confidence - 0.4).abs() < f32::EPSILON);
