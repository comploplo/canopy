...
impl CacheableData for VerbNetData {
...
impl VerbNetEngine {
    ...
    ///
    /// # Errors
    /// Returns an error if `VerbNet` data path does not exist or data cannot be loaded.
    ...
    pub fn with_config(verbnet_config: VerbNetConfig) -> EngineResult<Self> {
        // Convert VerbNetConfig to EngineConfig using trait
    ...
    fn calculate_verb_confidence(classes: &[VerbClass]) -> f32 {
        if classes.is_empty() {
            return 0.0;
        }
        ...
        let avg_specificity = classes
            ...
            .map(|c| {
                ...
                let specificity = count_to_f32(c.id.matches('-').count()) * 0.1;
                // Classes with more frames are more informative
                let frame_bonus = (count_to_f32(c.frames.len()) * 0.05).min(0.2);
                specificity + frame_bonus
            })
            .sum::<f32>()
            / count_to_f32(classes.len());
...
impl VerbNetEngine {
    ...
    ///
    /// # Errors
    /// Returns an error if the test data path does not exist or contains invalid data.
    #[cfg(test)]
    ...
    ///
    /// # Errors
    /// Returns an error if the directory does not exist or contains no valid XML files.
    ...
    #[must_use]
    pub fn get_stats(&self) -> crate::engine::EngineStats {
        self.base_engine.get_stats()
    }

    /// Get performance metrics from `BaseEngine`
    #[must_use]
    pub fn get_performance_metrics(&self) -> crate::engine::PerformanceMetrics {
        self.base_engine.get_performance_metrics()
    }

    /// Get quality metrics from `BaseEngine`
    #[must_use]
    pub fn get_quality_metrics(&self) -> crate::engine::QualityMetrics {
    ...
    #[must_use]
    pub fn statistics(&self) -> crate::engine::EngineStats {
        self.get_stats()
    }

    /// Get performance metrics (alias for `get_performance_metrics`)
    #[must_use]
    pub fn performance_metrics(&self) -> crate::engine::PerformanceMetrics {
    ...
    ///
    /// # Errors
    /// Always returns an error because test data loading is not implemented.
...
mod tests {
    ...
    fn test_confidence_calculation() {
        ...
        let _engine = engine_ref.lock().unwrap();

        // Test empty classes
        assert!(VerbNetEngine::calculate_verb_confidence(&[]).abs() < f32::EPSILON);
        ...
        let confidence = VerbNetEngine::calculate_verb_confidence(&single_class);
