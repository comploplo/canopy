...
use super::types::{
    ...
    SelectionalRestrictions, SemanticPredicate, SyntaxElement, SyntaxPattern, ThematicRole,
    VerbClass,
};
use crate::engine::{extract_text_content, get_attribute, EngineError, EngineResult, XmlResource};
...
fn parse_syntax<R: BufRead>(
    ...
    loop {
        match reader.read_event_into(buf) {
            Ok(Event::Start(ref e)) => {
                if let QName(b"NP" | b"VERB" | b"PREP" | b"ADJ" | b"ADV") = e.name() {
...
fn extract_class_name(class_id: &str) -> String {
    ...
    if let Some(dash_pos) = class_id.find('-') {
        class_id[..dash_pos].replace('_', " ").clone()
    } else {
        class_id.replace('_', " ").clone()
    }
}
...
mod tests {
    ...
    fn test_thematic_role_restrictions() {
        // Example XML structure this test validates:
        // <THEMROLE type="Agent">
        //     <SELRESTRS logic="or">
        //         <SELRESTR type="animate" Value="+"/>
