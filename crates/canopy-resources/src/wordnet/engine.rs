...
use super::loader::WordNetLoader;
use super::parser::WordNetParserConfig;
use super::types::{PartOfSpeech, WordNetAnalysis, WordNetDatabase};
use crate::engine::{
    count_to_f32, BaseEngine, CacheKeyFormat, CacheableData, CachedEngine, DataInfo, DataLoader,
    EngineConfigurable, EngineCore, EngineResult, EngineStats, PerformanceMetrics, SemanticEngine,
    SemanticResult, StatisticsProvider,
};
...
impl CacheableData for WordNetData {
...
crate::impl_engine_configurable!(WordNetConfig, min_confidence);
...
impl WordNetEngine {
    ...
    ///
    /// # Errors
    /// Returns an error if `WordNet` data path does not exist or data cannot be loaded.
    ...
    pub fn with_config(wordnet_config: WordNetConfig) -> EngineResult<Self> {
        // Convert WordNetConfig to EngineConfig using trait
        let engine_config = wordnet_config.to_engine_config();
        ...
        if let Ok(offset) = synset_id.parse::<usize>() {
            if let Some(synset) = self.database.get_synset(offset) {
                return self
                    ...
                    .into_iter()
                    .filter_map(|s| s.primary_word().map(std::string::ToString::to_string))
                    .collect();
            }
        }
    ...
    fn calculate_wordnet_confidence(analysis: &WordNetAnalysis) -> f32 {
        if analysis.synsets.is_empty() {
            return 0.0;
        }

        // Base confidence from synset count
        let synset_count_factor = (count_to_f32(analysis.synsets.len()) * 0.1).min(0.5);

        // Relations factor (more relations indicate richer semantic data)
        let relations_factor = (count_to_f32(analysis.relations.len()) * 0.05).min(0.2);
            ...
            for synset in &analysis.synsets {
                ...
                if !hypernyms.is_empty() {
                    analysis.relations.push((
                        super::types::SemanticRelation::Hypernym,
                ...
                if !hyponyms.is_empty() {
                    analysis.relations.push((
                        super::types::SemanticRelation::Hyponym,
...
mod tests {
    ...
    fn wordnet_available() -> bool {
        crate::paths::data_path("data/wordnet").exists()
    }
    ...
    fn test_confidence_varies_with_richness() {
        ...
        let empty_confidence = WordNetEngine::calculate_wordnet_confidence(&empty_analysis);
        ...
        let rich_confidence = WordNetEngine::calculate_wordnet_confidence(&result.data);

        // Rich analysis should have higher confidence than empty
        assert!(
            rich_confidence > empty_confidence,
            "Word with synsets ({rich_confidence}) should have higher confidence than empty analysis ({empty_confidence})"
        );
        // Rich analysis should have meaningful confidence
        assert!(
            rich_confidence >= 0.3,
            "Common verb 'run' should have reasonable confidence, got {rich_confidence}"
        );
    }
}
