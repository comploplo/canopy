...
canopy_engine::impl_engine_configurable!(FrameNetConfig);
...
impl FrameElement {
    ...
    /// Convert frame element name to canonical ThetaRole
    ///
    /// Uses the centralized role string mapping from canopy-core.
    /// Returns None if the frame element name doesn't map to a known role.
    pub fn to_theta_role(&self) -> Option<ThetaRole> {
        ThetaRole::from_role_str(&self.name)
    }
}

impl From<&FrameElement> for Option<ThetaRole> {
    fn from(fe: &FrameElement) -> Self {
        fe.to_theta_role()
    }
}
