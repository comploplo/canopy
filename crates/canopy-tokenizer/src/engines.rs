...
mod tests {
    use super::*;
    use crate::test_support::{with_framenet, with_multi_resource_analyzer, with_verbnet};

    #[test]
    fn test_semantic_engine_trait_verbnet() {
        let Some(result) = with_verbnet(|engine| engine.analyze_token("run")) else {
            eprintln!("Skipping test: semantic data not available");
            return;
        };

        // VerbNet should have data for common verbs
        let classes = result.expect("VerbNet should analyze 'run'");
        // "run" is a common verb that should have VerbNet classes
        assert!(!classes.is_empty(), "VerbNet should find classes for 'run'");
    }

    #[test]
    fn test_semantic_engine_trait_framenet() {
        let Some(result) = with_framenet(|engine| engine.analyze_token("give")) else {
            eprintln!("Skipping test: semantic data not available");
            return;
        };

        // FrameNet should be able to analyze common verbs
        assert!(
            result.is_ok(),
            "FrameNet should successfully analyze 'give'"
        );
    }
    ...
    fn test_multi_resource_analyzer() {
        let Some(result) =
            with_multi_resource_analyzer(|analyzer| analyzer.analyze_comprehensive("give"))
        else {
            eprintln!("Skipping test: semantic data not available");
            return;
        };

        // "give" is a common verb that should have semantic data
        let result = result.expect("analyze_comprehensive should succeed");
        assert!(
            !result.sources.is_empty(),
            "Should find semantic sources for 'give'"
        );
    }

    #[test]
    fn test_coverage_stats() {
        let Some(stats) = with_multi_resource_analyzer(|analyzer| {
            let test_lemmas = vec!["give".to_string(), "walk".to_string(), "book".to_string()];
            analyzer.get_coverage_stats(&test_lemmas)
        }) else {
            eprintln!("Skipping test: semantic data not available");
            return;
        };

        // With engines loaded, we should have some coverage for common words
        assert!(stats.coverage_percentage >= 0.0);
        // "give" and "walk" are common verbs, should have coverage
        assert!(
            stats.total_covered_lemmas >= 1,
            "Should find at least one covered lemma"
        );
    }
    ...
    fn test_unified_statistics() {
        let Some(stats) = with_multi_resource_analyzer(|analyzer| {
            ...
            analyzer.get_unified_statistics(&test_lemmas)
        }) else {
            eprintln!("Skipping test: semantic data not available");
            return;
        };

        // With engines loaded, VerbNet should have classes for common verbs
        // "give" is VerbNet class give-13.1, "walk" is walk-51.3
        assert!(
            stats.verbnet.total_classes >= 1,
            "VerbNet should have classes for common verbs"
        );
    }
}
