...
fn test_lemmatization_performance_integration() {
    ...
    // With lemmatization, should still be reasonably fast
    // In debug mode + coverage instrumentation, performance can be 50-100x slower than release
    // Release: ~100μs, Debug: ~1000-5000μs, Coverage: ~5000-10000μs
    // Use a generous threshold that works for all modes while still catching real regressions
    let threshold = if cfg!(debug_assertions) {
        50000.0 // Debug/coverage mode: generous threshold
    } else {
        5000.0 // Release mode: stricter threshold
    };
    assert!(
        avg_per_word < threshold,
        "Average time per word: {:.1}μs (threshold: {:.0}μs)",
        avg_per_word,
        threshold
    );
