...
V2 represents a complete architectural redesign of spaCy-LSP, moving from
Python's surface-level semantic mapping to a theoretically-grounded linguistic
analysis platform in Rust. The redesign prioritizes explicit feature extraction,
formal linguistic theory, and compile-time type safety while maintaining LSP
integration.

**Core Transformation**: From `spaCy → JSON → Proto → LSP` to
`UDPipe → Events → DRT → LSP` with typed, theory-driven intermediate
representations.
...
1. **Type Safety First**: Use Rust's type system to enforce linguistic
   constraints at compile time
2. **Theory-Driven Design**: Every architectural decision grounded in linguistic
   theory
3. **Explicit Over Implicit**: No hidden features or black-box processing
4. **Compositionality**: Each layer's output is the next layer's well-typed
   input
5. **Incremental Complexity**: Start simple, add complexity only where needed
6. **Performance Through Design**: Efficiency from architecture, not
   micro-optimization
...
- **Python API**: Use PyO3 for seamless Python bindings to Rust core components
...
V2 represents a fundamental redesign that brings formal linguistic theory to
practical NLP tooling. By leveraging Rust's type system and implementing
established linguistic frameworks (UD, DRT, OT), we create a system that is both
theoretically sound and practically efficient. The modular architecture ensures
that each component can evolve independently while maintaining type safety
across layer boundaries.

The key innovation is making linguistic theory computational and practical,
bridging the gap between theoretical linguistics and software engineering. This
positions spaCy-LSP V2 not just as a better implementation, but as a new
category of tool: a theory-aware, type-safe linguistic analysis platform.

- Please add a TODO when code is simplified but for immediate needs but needs
  more expansion later.

---

## Coverage Requirements and Maintenance

### Mandatory Coverage Rules

1. **New Source Code Rule**: All new code in `src/` directories MUST include
   tests that achieve at least 80% line coverage for the new code.

2. **Refactoring Signal**: If code cannot achieve 80% coverage due to
   complexity, it MUST be refactored into smaller, testable units.

3. **Pre-commit Enforcement**: The coverage gate of 80% is enforced
   automatically - commits will fail if coverage drops below this threshold.

4. **Coverage Gate Preservation**: The coverage gate must NEVER be lowered or
   bypassed during releases. Maintain integrity of quality standards.

### Coverage Maintenance Guidelines

- **Test-First Development**: Write tests alongside new functionality, not as an
  afterthought
- **Coverage Check Before PR**: Run `just coverage-check` before submitting pull
  requests
- **Complexity as Code Smell**: Untestable code indicates design problems that
  need architectural fixes
- **Split Complex Functions**: Break down functions that are hard to test into
  smaller, focused units
- **Mock External Dependencies**: Use dependency injection to make code testable
  and eliminate external dependencies in tests
- **Test Real Behavior**: Focus on testing actual functionality, not just
  achieving line coverage numbers

### Coverage Targets by Milestone

- **M3 Target**: 80% minimum for presubmit (ACTIVE - DO NOT LOWER)
- **M4 Target**: 90% minimum for presubmit
- **New Features**: 95% coverage requirement for all new functionality
- **Critical Paths**: 100% coverage for error handling, security, and core
  parsing logic
...
```bash
# Automatic coverage check on every commit
just coverage-check  # Must pass 80% threshold

# Check coverage for new code specifically
scripts/check-new-code-coverage.sh
```
...
just coverage-module canopy-lsp

# Identify untested code lines
just coverage-gaps
...
Coverage metrics serve quality, not the reverse:

- **80% minimum ensures basic functionality is tested**
- **90% target ensures comprehensive edge case coverage**
- **100% critical path coverage ensures system reliability**
- **Focus on meaningful tests that validate actual behavior**
- **Avoid "test theater" - tests that execute code without validating
  correctness**

### Code Complexity and Testability

If achieving 80% coverage proves difficult, consider these refactoring
strategies:

1. **Extract Pure Functions**: Move logic to functions with no side effects
2. **Dependency Injection**: Make external dependencies mockable
3. **Smaller Modules**: Break large modules into focused components
4. **Error Handling Separation**: Extract error handling to testable units
5. **Configuration Externalization**: Make behavior configurable rather than
   hardcoded

**Remember**: Untestable code is often poorly designed code. Use testability as
a guide for better architecture.
...
**Performance Achievement**: 7-76μs per sentence (16,000x improvement over 10ms
target!) **Implementation**: UDPipe-first feature extraction + unified semantic
features + comprehensive testing **Result**: M2 completely finished - ready for
M3!
...
- ✅ UDPipe-first feature extraction (12 morphological features + unified
  system)
- ✅ Comprehensive golden test validation (6 tests covering accuracy and
  performance)
...
- **Production Model**: UDPipe 1.2 validated at 1.56ms latency, 641 sent/sec, 0%
  error rate
...
- **UDPipe Features**: 12 morphological features (animacy, voice, aspect, tense,
  number, etc.)
...
### 🎯 **M2 → M3 Transition: IN PROGRESS**

**M2 Foundation Complete**:

- ✅ **UDPipe 1.2 production model** - 1.56ms latency, 641 sent/sec, 0% error
  rate
...
- ✅ **EventBuilder pattern** implemented for clean Event construction -
  COMPLETE
- ✅ **19 theta role inventory** verified from Python V1 system - COMPLETE
- ✅ **MovementChain representation** integrated in Event structure - COMPLETE
- ✅ **LittleV decomposition** (Cause, Become, Do, Be, Go, Have) - COMPLETE
- 🎯 **VerbNet theta role assignment** (leveraging M2's unified feature
  system) - IN PROGRESS
- 🎯 **Movement signal detection** (passive, raising patterns) - PLANNED

**Performance Headroom Confirmed**:

- **Current**: 1.56ms UDPipe 1.2 baseline (production-ready)
- **Target**: <500μs total analysis (tokenizer compatibility)
- **Available**: Massive budget for semantic analysis with excellent baseline
