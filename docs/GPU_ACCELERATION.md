...
canopy.rs achieves 25-80μs per sentence on CPU. GPU compute shaders can provide
O(n) speedup for batch processing while maintaining single-sentence performance,
potentially reaching <1μs per sentence for large batches.

**Key Insight**: Move VerbNet analysis and semantic operations to GPU for
massive parallelism opportunities.
