    ...
    for (i, sentence) in test_sentences.iter().enumerate() {
        ...
        if let Some(sentence_analysis) = response.document.sentences.first() {
            ...
            for word in &sentence_analysis.words {
                ...
                let morph_info = if word.feats.person.is_some()
                    || word.feats.number.is_some()
                    || word.feats.tense.is_some()
                {
                ...
                println!(
                    "       {} {} → {} ({:?}){}",
                    pos_emoji, word.text, word.lemma, word.upos, morph_info
                );
            }
        }
